...
var { clientId,JWT_SIGN_KEY ,JWT_SECRET_KEY} =require( '../properties');
...
exports.googleLogin=(req,res)=>{
   ...
   client.verifyIdToken({idToken:token,audience:clientId}).then(response=>{
            ...
            if(user)
            {
                ...
                userdetail.save((err,response)=>{
                    if(err)
                    {
                       ...
                       const resetURL=`http://localhost:3001/verify/${userdetail.token}`;
                       try {
       ...
       if(user)
            {
                ...
                const resetURL=`http://localhost:3001/verify/${user.token}`;
                try {sendEmail({
                     to:user.email,
                     subject:"Email verification link",
                     text:`<h4> email verification link</h4>
